...
import { isNumber, numberToCosmosSdkDecString, snakeToPascal } from '../utils'
    ...
    if (type === 'boolean') {
      ...
      if (Array.isArray(propertyValue) && propertyValue.length === 0) {
        ...
        if (isPrimitive) {
          ...
          Array.from(recursiveOutput.keys()).forEach(key => {
            if (key !== primaryType) {
...
export const mapValuesToProperValueType = <T extends Record<string, unknown>>(
  ...
  return Object.keys(object).reduce((result, key) => {
    ...
    if (!value) {
      ...
      // if value is bigint or number 0, return that value
