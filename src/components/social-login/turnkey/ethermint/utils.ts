    ...
    signerInfos: [
      {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sequence: signerInfo.sequence.toString() as any
      }
    ],
    fee: {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      gasLimit: fee.gasLimit.toString() as any,
      payer: fee.payer
    }
  })
}
...
export function createProtoIBCMsgTransfer(
  ...
  return {
    value: MsgTransfer.encode(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      MsgTransfer.fromPartial(ibcMessage as any)
    ).finish(),
...
export function createTxIBCMsgTransfer(
  ...
  const signDirectDoc = SignDoc.encode(
    SignDoc.fromPartial({
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      accountNumber: sender.accountNumber as any,
...
export function createProtoMsgSend(
  fromAddress: string,
  toAddress: string,
  amount: string,
  denom: string
...
export function createMessageSend(
  ...
  const eipToSign = createEIP712Tx(eip712Types, chain.chainId, messages)
  const msgCosmos = createProtoMsgSend(
    sender.accountAddress,
    params.destinationAddress,
    params.amount,
    params.denom
...
export function createProtoMsgVote(
  proposalId: number,
  option: number,
  sender: string
) {
  ...
  return {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    value: MsgVote.encode(MsgVote.fromPartial(message as any)).finish(),
...
export function createTxMsgVote(
  ...
  const eipToSign = createEIP712Tx(eip712Types, chain.chainId, messages)
  const msgCosmos = createProtoMsgVote(
    params.proposalId,
    params.option,
    sender.accountAddress
  )
...
export function createMsgGrant(
  ...
  return {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    value: MsgGrant.encode(MsgGrant.fromPartial(message as any)).finish(),
...
export function createStakeAuthorization(
  ...
  return {
    value: StakeAuthorization.encode(
      StakeAuthorization.fromPartial(message)
    ).finish(),
...
export function createProtoMsgWithdrawDelegatorReward(
  delegatorAddress: string,
  validatorAddress: string
) {
  ...
  return {
    value: MsgWithdrawDelegatorReward.encode(
      MsgWithdrawDelegatorReward.fromPartial(message)
    ).finish(),
...
export function createTxMsgMultipleWithdrawDelegatorReward(
  ...
  params.validatorAddresses.forEach((validator) => {
    ...
    msgsCosmos.push(
      createProtoMsgWithdrawDelegatorReward(sender.accountAddress, validator)
    )
  })
...
export function createProtoMsgBeginRedelegate(
  ...
  return {
    value: MsgBeginRedelegate.encode(
      MsgBeginRedelegate.fromPartial(message)
    ).finish(),
